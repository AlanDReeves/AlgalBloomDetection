...
# This version takes in data, feeds it to a 4 parameter model predicting water or land, and then adds that to the data
...
modelPath = filedialog.askopenfilename(defaultextension='.csv', title='Select water detection model file')
model: Algae_Bloom_Model4 = load(modelPath)
model.print_stats()


tif_folder = "workingTifs"
output_csv = "erie_algal_bloom_with_isWater.csv"
...
with open ('band_box2.csv') as lat_long_file:
...
for row in range(row_top, row_bottom + 1):
    for col in range(col_left, col_right + 1):
        ...
        values[5] = lat
        values[6] = lon
        values[7] = model.predict_one_line([values[:4]])[0][1] # selects specificallyl the probability that a pixel is water
        print(f"Prediction: {values[7]}")
